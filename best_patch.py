...
def patch_distance(patch_indices,img,patch_size,source_indices_complete,source_region):
    max_similarity=[0,0]
    min_dist=10000000000
    target_patch = img[patch_indices[0]:patch_indices[1], patch_indices[2]:patch_indices[3]]
    ...
    for point in source_indices_complete:
        p_x, p_y = point
        source_patch = img[p_y - patch_size:p_y + patch_size + 1, p_x - patch_size:p_x + patch_size + 1]
        mask = source_region_patch == 255
        difference = source_patch.astype(float) - target_patch.astype(float)
        sum_difference = np.sum(np.square(difference[mask]))
        sum_difference /= np.count_nonzero(mask)
